...
print(f"BS SIMDT {bs.settings.simdt}")
...
for reception_prob in tqdm([0.6, 0.7, 0.9], desc="Reception prob"):
    for lookahead_time in tqdm([5, 10, 15, 20], desc="Lookahead time"):
        ...
        for init_speed_intruder in tqdm([20], desc="Intruder Speeds"):
            ...
            for dpsi in tqdm([2, 4, 6, 10, 20, 30, 45, 60, 90, 120, 150, 180], desc=f"DPSI for SPD {init_speed_intruder}", leave=False):
            # for dpsi in tqdm(range(2, 4, 2), desc=f"DPSI for SPD {init_speed_intruder}", leave=False):
                ...
                if(dpsi <= 2):
                    nb_of_repetition = 100
                    tmax = 30 * lookahead_time
                    minsimtime = tmax / 30

                if(lookahead_time <= 10):
                    tmax *= 3
