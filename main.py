...
spd_uncertainty_sigma = 3 if 's' in nav_uncertainty else 0
hdg_uncertainty_sigma = 5 if 'h' in nav_uncertainty else 0
pos_uncertainty_sigma = 15 if 'p' in nav_uncertainty else 0
vel_uncertainty_sigma = 1.5 if 'v' in nav_uncertainty else 0
...
for init_speed_intruder in tqdm([5, 15, 20], desc="Intruder Speeds"):
    ...
    for dpsi in tqdm(range(0, 45, 2), desc=f"DPSI for SPD {init_speed_intruder}", leave=False):
        los_list = []
        distance_cpa_list = []
        ...
        for rep in range(nb_of_repetition):  # Changed _ to rep to track repetition number
    ...
    df.to_csv(
        f'results/results_{fname}_{init_speed_intruder}_{pos_uncertainty_sigma}_{vel_uncertainty_sigma}.csv',
        index=False)