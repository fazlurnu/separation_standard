                    ...
                    for dpsi in tqdm(range(2, 41, 2), desc=f"DPSI"):
                        ...
                        # for rep in range(nb_of_repetition):
                            ...
                            while sim_timer_second < tmax and True:
                                ...
                                # Initialize ADSL models (if not yet done)
                                if sim_timer_second == 0:
                                    ...
                                    adsl.send_data(intruder_adsl, ownship_adsl, None)
                                    adsl.send_data(prev_intruder_adsl, intruder_adsl, None)  # store t=0 snapshot
                                ...
                                if ((round(sim_timer_second, 2) % bs.settings.asas_dt == 0)):
                                    ...
                                    if update_indices.size > 0:
                                        adsl.send_data(intruder_adsl, ownship_adsl, update_indices)

                                    # Fill in missed aircraft with previous state
                                    if missed_indices.size > 0:
                                        adsl.send_data(intruder_adsl, prev_intruder_adsl, missed_indices)

                                    # Store this state for next tick fallback
                                    adsl.send_data(prev_intruder_adsl, intruder_adsl)
                            ...
                            # trajectory_df.to_csv(f"{dpsi_dir}/trajectory_{fname}_{reception_prob}_{init_speed_intruder}_{pos_uncertainty_sigma}_{vel_uncertainty_sigma}_{rep}_{lookahead_time}.csv", index=False)
